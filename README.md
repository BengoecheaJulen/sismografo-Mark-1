...
|-------|-------|--------------------|
| 🟣 `#9C27B0` | **EXTREME TAIL** | 5+ consecutive CRITICAL sessions. Systemic cascade risk. |
| 🔴 `#FF1744` | **CRITICAL** | NEXUS > 0.50 AND APEX > 0.30 |
| 🟠 `#FF3D00` | **HIGH RISK** | NEXUS > 0.35 sustained, APEX rising |
| 🟡 `#FFB300` | **ELEVATED** | NEXUS between 0.25 and 0.35, monitoring mode |
| 🟢 `#00E676` | **STABLE** | Normal market regime, no anomalies detected |
| 🔵 `#00B0FF` | **SILENT BUBBLE** | NEXUS > 0.40 AND APEX < 0.15 AND VECTOR < −0.15 (3+ days) |
| 💜 `#E040FB` | **EXTREME BUBBLE** | NEXUS > 0.45 AND APEX < 0.10 AND VECTOR < −0.20 (3+ days) |
...
- **Calibration period:** 2000–2021 (5,286 observations)
- **Validation period:** 2022–2025 out-of-sample (1,031 observations, model never saw this data)
...
|--------|-------|----------------|
| Validation RMSE / Training RMSE | **0.84** | Model performs 16% *better* on unseen data than on training data — opposite of overfitting |
| Parameters | **11** | Minimal parametrization for a complex system |
| Training observations | **5,286** | 480:1 data-to-parameter ratio (minimum accepted: 10:1) |
| Validation correlation (Pearson) | **0.607** | Strong signal in out-of-sample period |
| Validation correlation (Spearman) | **0.613** | Robust to non-linear distortions |

A val/train RMSE ratio below 1.0 is statistically unusual and indicates the model has learned genuine market structure rather than noise. The 480:1 data-to-parameter ratio is 48× above the standard econometric minimum, providing high confidence in parameter stability.
