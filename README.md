...
The engine runs entirely in the browser (`index.html`). All percentile thresholds are computed from the full 2000–2026 historical dataset and are fixed — no threshold has been adjusted to match recent data.

**State hierarchy (evaluated top to bottom, first match wins):**

1. **VOID** — 5+ consecutive days in CRITICAL
2. **CRITICAL** — Λ > P85 AND γ < P50 (historical mean σ = 0.274)
3. **HIGH** — Λ > P70 OR (Λ > P50 AND γ < P25) (historical mean σ = 0.202)
4. **SYNC** — Cq > P90 AND Λ < P70 AND γ > P50 (unusual coherence without stress, σ = 0.126)
5. **ELEVATED** — Λ > P50 OR γ < P50 (historical mean σ = 0.147)
6. **STABLE** — default below ELEVATED thresholds (historical mean σ = 0.123)
7. **CALM** — Λ < P30 AND γ > P75 (historical mean σ = 0.095)
